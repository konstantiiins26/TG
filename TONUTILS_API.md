...
**Внедрено 18.09.2026** — см. `_send_purchase()` в `gift_sniper.py`. Пункты 2
и 5 выше закрыты: `sale_address` сохраняется парсером, `REAL_EXECUTOR_AVAILABLE`
определяется успехом импорта.

Живьём НЕ проверено: ни одной транзакции ещё не отправлено. Первый прогон —
только testnet.
