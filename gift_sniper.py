...
def _collect_sample(collection: str):
    ...
    `exhausted = True` означает, что мы видели коллекцию ЦЕЛИКОМ, и только
    при нём можно утверждать что-либо про САМЫЙ ДЕШЁВЫЙ лот сегмента.

    ЧТО СЧИТАЕТСЯ ДОКАЗАТЕЛЬСТВОМ КОНЦА — переписано 23.09.2026 после
    Liberty Figure #80594. Бот написал «следующий Astronaut стоит 10.00»,
    а на витрине при фильтре по Astronaut стояли 6.94, 7, 7, 8, 8, 8, 8, 9
    и только потом 10. Прибыль +3.94 TON была выдумкой: настоящая +1.03.

    Ворота `exhausted` при этом СТОЯЛИ и пропустили лот. Потому что
    прежнее правило гласило «обход упёрся в ПУСТУЮ страницу → коллекция
    кончилась», и это было не измерение, а ДОПУЩЕНИЕ О ПОВЕДЕНИИ TonAPI,
    записанное словом «измерение». Пустая страница означает ровно одно:
    сервер не дал предметов по этому смещению. Причин у этого несколько —
    конец коллекции, потолок смещения на стороне API, сбой под нагрузкой,
    — и различить их по самому ответу нельзя.

    В том же прогоне видно и второе лицо той же беды: на `offset=20000`
    пришёл `500 Server Error`, то есть около двадцати тысяч смещение
    перестаёт обслуживаться. Там, где вместо 500 приходит пустой список,
    прежний код объявлял покрытие 100%.

    НОВОЕ ПРАВИЛО: конец доказывает НЕПОЛНАЯ страница, а не пустая.
    Сервер вернул меньше `FLOOR_PAGE_SIZE` предметов — значит по этому
    смещению у него их меньше, чем мы просили, и дальше ничего нет. Совпасть
    случайно это не может: чтобы конец коллекции пришёлся ровно на границу
    страницы, её размер должен быть кратен 1000.

    И даже неполная страница ПРОВЕРЯЕТСЯ ещё одним запросом за ней: под
    нагрузкой сервер может отдать укороченный ответ, и тогда «неполная»
    означает не конец, а обрезку. Один лишний запрос на коллекцию за цикл —
    цена того, чтобы утверждение о покрытии было измеренным.

    Пустая страница обход ОСТАНАВЛИВАЕТ (дальше тянуть нечего), но
    покрытие не подтверждает. Практическое следствие названо вслух: у
    коллекций крупнее потолка смещения TonAPI наблюдение по сегменту будет
    молчать всегда. Это честно — раньше оно на том же месте выдавало
    прибыль из воздуха.
    ...
    """
    # Смещение ведётся ОТДЕЛЬНЫМ счётчиком, а не как page*FLOOR_PAGE_SIZE:
    ...
    for _page in range(FLOOR_SAMPLE_PAGES):
        try:
            ...
            failures += 1
            log.warning(f"TonAPI: страница {_page + 1}/{FLOOR_SAMPLE_PAGES} "
                        f"(offset={offset}) не получена ({e}).")
            offset += FLOOR_PAGE_SIZE      # пропускаем битую страницу, не топчемся
            continue

        if not batch:
            # НЕ доказательство конца: так же выглядит потолок смещения у
            # API и сбой под нагрузкой. Обход прекращаем, покрытие НЕ
            # подтверждаем.
            log.info(f"{_Color.GREY}{_short(collection)}: пустая страница на "
        ...
        if len(batch) < FLOOR_PAGE_SIZE:
            ...
            # ответ под нагрузкой выглядит так же.
            try:
                tail = fetch_items_tonapi(collection, FLOOR_PAGE_SIZE,
                                          offset=offset)
            except Exception as e:  # noqa: BLE001
            ...
            if tail:
                ...
                log.warning(f"{_short(collection)}: страница пришла неполной "
                            f"({len(batch)} из {FLOOR_PAGE_SIZE}), но за ней "
...
# Порог ROI для НАБЛЮДЕНИЯ по сегменту — отдельный от торгового MIN_ROI_PCT
# (5%). Владелец 23.09.2026: «кидай от 10». Торговлю это не трогает: её
# порог — вопрос экономики сделки, а этот — вопрос того, что стоит показывать
# в телефоне. Поднять порог показа безопасно по определению: он только
# УБАВЛЯЕТ утверждения.
SEGMENT_MIN_ROI_PCT = Decimal(os.getenv("SEGMENT_MIN_ROI_PCT", "10"))

# ПОРОГА ПО ЧИСЛУ КОНКУРЕНТОВ ЗДЕСЬ НЕТ, И ЭТО ПРОВЕРЕНО, А НЕ ЗАБЫТО.
# 23.09.2026 он был поставлен (15 лотов) и в тот же час снят: два живых
# случая одного вечера его НЕ РАЗДЕЛЯЮТ.
#
#   Timeless Books #72993  сегмент 5 лотов  -> вывод НЕВЕРЕН (на витрине
#                                              три лота дешевле нашего)
#   Surge Boards  #20176   сегмент 6 лотов  -> вывод ВЕРЕН (следующий
#                                              Celestial правда 15.00)
#
# Пять против шести. Любой порог между ними — выдуманный критерий, а порог
# выше убивает ВЕРНУЮ находку: у модели Celestial на витрине всего 11
# выставленных, и 15 она не наберёт никогда.
#
# Различает эти два случая не размер сегмента, а ПОЛНОТА ОБХОДА: увидели ли
# мы витрину целиком. Именно это проверяют ворота `exhausted`, и лечится оно
# лимитом TonAPI, а не ещё одним числом здесь.

# ПОТОЛОК ПРАВДОПОДОБИЯ. Слишком высокий ROI — это не находка века, а признак
# того, что выборка врёт. Проверено на живых случаях этого проекта:
#
#   Pool Floats  #141416  ROI 163%  -> обход оборвался лимитом, конкуренты
#                                      по 5.00 в выборку не попали
#   Liberty Fig. #80594   ROI  72%  -> «конец коллекции» был пустой страницей
#   Candy Canes  #105655  ROI  ~60% -> перцентиль по сегменту из 19 лотов
#
# Три случая, все с высоким ROI, все оказались ошибкой данных. Настоящих
# находок такого размера не подтвердилось НИ ОДНОЙ. Поэтому лот выше потолка
# в телефон не идёт — но в лог идёт целиком и попадает в сводку: подавленное
# молча неотличимо от несуществующего (тот же довод, что у подавленных
# находок).
#
# Это НЕ увеличение расчётной прибыли и не ослабление ворот: механизм только
# убирает утверждения, в которые у нас нет оснований верить.
SEGMENT_MAX_ROI_PCT = Decimal(os.getenv("SEGMENT_MAX_ROI_PCT", "100"))

# Сколько лотов отсеял потолок правдоподобия за час — в сводку.
_segment_implausible = 0
_segment_seen: dict = {}
...
def cheapest_rival(item: dict, peer_prices: dict):
    """
    Цена САМОГО ДЕШЁВОГО КОНКУРЕНТА в сегменте: (цена | None, сколько всего).

    Заменяет перцентиль в пути наблюдения, и вот почему.

    Живой прогон 23.09.2026, Candy Canes #105655. Модель Toxicane: 19
    выставленных лотов, самый дешёвый 4.47 (это и есть наш лот), следующий
    около 11. Перцентиль P5 при N=19 попадает на индекс (19-1)x0.05 = 0.90,
    то есть НА 90% ПУТИ ОТ ПЕРВОГО КО ВТОРОМУ, и интерполяция выдала 10.35.

    Витрина Getgems при этом писала «Toxicane 4.47 GRAM floor». Владелец
    сверил и спросил, почему floor неправильный. Он прав: 10.35 — это не
    цена, по которой кто-то что-то выставил, а точка В ПУСТОТЕ между двумя
    настоящими ценами. Чем реже сегмент, тем дальше эта точка от реальности.

    Проект уже знал этот эффект: `MIN_FLOOR_SAMPLE = 40` существует ровно
    потому, что ниже сорока P5 опирается на один-два лота. Но к СЕГМЕНТУ
    урок не применили — там порог остался 4.

    Берётся минимум по ЧУЖИМ лотам: свой из выборки исключается. Сравнивать
    лот со статистикой, в которую он сам входит, — круг: наш лот тянул
    «floor сегмента» вниз, а потом объявлялся дешевле него на 56.8%.

    Это НАБЛЮДЁННАЯ цена, а не вычисленная, и она же определяет, сможем ли
    мы продать: чтобы уйти, надо встать ниже самого дешёвого конкурента.

    ЧЕСТНО ПРО НАПРАВЛЕНИЕ. Когда наш лот НЕ самый дешёвый, эта цена ниже
    прежнего перцентиля — оценка падает. Когда наш лот самый дешёвый, она
    выше: перцентиль интерполировал к второму лоту, а мы берём сам второй
    лот. Рост ограничен реальной ценой на витрине, а не формулой, — но это
    ЦЕНА ПРОСЬБЫ, а не сделки, и в сообщении это сказано.
    """
    value = (item.get("traits") or {}).get(PEER_TRAIT)
    if not value:
        return None, 0
    prices = [Decimal(str(p)) for p in (peer_prices or {}).get(str(value), [])]
    if not prices:
        return None, 0
    total = len(prices)

    # Убираем ОДНО вхождение своей цены: в сегменте могут стоять несколько
    # лотов по той же цене, и вычёркивать их все значит выкинуть конкурентов.
    own = Decimal(str(item["sale_price_ton"]))
    rivals = list(prices)
    if own in rivals:
        rivals.remove(own)
    if len(rivals) < MIN_PEER_SAMPLE:
        return None, total
    return min(rivals), total
...
def find_segment_bargains(snap: dict):
    ...
    for item in snap.get("on_sale", []):
        if not is_collection_trusted(item.get("collection_address", "")):
            continue
        # НЕ перцентиль: при малой выборке он садится между двумя реальными
        # ценами и выдаёт точку в пустоте (см. cheapest_rival). Торговый путь
        # это НЕ затрагивает — `evaluate_trade()` по-прежнему зовёт
        # `peer_floor()`, и решение о покупке считается как раньше.
        seg_floor, peer_n = cheapest_rival(item, snap.get("peer_prices"))
        ...
        if profit <= 0 or roi < SEGMENT_MIN_ROI_PCT:
            continue
        # Потолок правдоподобия: у всех трёх разобранных случаев с ROI выше
        # сотни причиной была НЕПОЛНАЯ ВЫБОРКА, а не дешёвый лот. В лог такой
        # лот идёт целиком — решение остаётся за владельцем, — но в телефон
        # нет: там каждое сообщение читается как утверждение бота.
        if SEGMENT_MAX_ROI_PCT > 0 and roi > SEGMENT_MAX_ROI_PCT:
            global _segment_implausible
            _segment_implausible += 1
            log.warning(
                f"{_Color.YELLOW}ROI {roi}% у «{(item.get('traits') or {}).get(PEER_TRAIT, '?')}» "
                f"{_short(item.get('address', ''))}: цена {buy}, ближайший "
                f"конкурент {seg_floor} по {peer_n} лотам выборки. Это выше "
                f"потолка правдоподобия {SEGMENT_MAX_ROI_PCT}% — втрое дешевле "
                f"одинаковых лотов рядом бывает от НЕПОЛНОЙ ВЫБОРКИ чаще, чем "
                f"от ошибки продавца. В телефон не шлю, проверьте руками "
                f"фильтром на витрине.{_Color.RESET}")
            continue
...
def notify_segment_bargain(b: dict) -> bool:
    ...
    lines = [
        ...
        f"🔵 ДЕШЕВЛЕ СВОИХ · на {b['discount_pct']}% ниже конкурента",
        f"🎁 {name}" + (f" #{mint}" if mint is not None else ""),
        f"💰 Купить {q(b['buy'])} → продать {q(sale_price)} → "
        f"📈 {q(b['profit']):+} TON ({b['roi']}%)",
        "",
    ]
    lines += price_chain_lines(b["buy"], b["seg_floor"],
                               "конкурента (самый дешёвый чужой лот)")
    lines.append("")
    lines += premium_lines(None, None, mint, False, b["discount_pct"],
                           f"следующего «{b['model']}»")
    ...
    lines += [
        ...
        # СВЕРКА С ВИТРИНОЙ. Getgems в фильтре по модели пишет «floor» и
        # показывает САМЫЙ ДЕШЁВЫЙ лот — а это как раз наш. Поэтому
        # сравниваем со СЛЕДУЮЩИМ: именно его придётся подрезать, чтобы
        # продать. Оба числа рядом, чтобы сверка глазами сходилась.
        f"📏 Сверка с витриной (фильтр по «{b['model']}»):",
        f"  на витрине floor {q(b['seg_cheapest'])} — это и есть наш лот",
        f"  следующий «{b['model']}» стоит {q(b['seg_floor'])} — его и режем",
        f"  всего выставлено {b['peer_n']} шт в нашей выборке",
        f"  коллекция: самый дешёвый {q(b['coll_cheapest'])}, "
        f"наш перцентиль {q(b['coll_floor'])}",
        "",
        # Цена конкурента — это ПРОСЬБА, а не сделка. Разница между ними и
        # есть неликвид, и у редкого сегмента она максимальна.
        f"ℹ️ {q(b['seg_floor'])} у конкурента — это ЦЕНА ПРОСЬБЫ, а не "
        f"цена сделки. Если по ней никто не покупает, продать выше не выйдет.",
        "",
...
def notify_heartbeat(snapshots: list, force: bool = False):
    ...
    if _segment_implausible:
        lines.append(f"⚠️ отсеяно по потолку правдоподобия "
                     f"(ROI > {SEGMENT_MAX_ROI_PCT}%): {_segment_implausible} "
                     f"— подробности в логе, проверять фильтром на витрине")
        _segment_implausible = 0
